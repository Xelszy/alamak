...
// GetPublisherBooks mengambil daftar buku berdasarkan publisher_id
func (r *BookRepository) GetPublisherBooks(publisherID uint) ([]dao.Book, error) {
	ctx, cancelFunc := storage.NewDBContext()
	defer cancelFunc()

	var books []dao.Book
	tx := r.db.WithContext(ctx).Where("publisher_id = ?", publisherID).
		Order("title").
		Find(&books)
